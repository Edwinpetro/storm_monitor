...
start_year = 2003 #datetime.now().year
...
# Use absolute paths
download_folder = os.path.join(script_dir, "forecast_data_parquet")
directory_path = os.path.join(script_dir, 'storm_adeck_directory.parquet')
...
def download_and_update_storm_data(file_url, save_path, directory_path):
    """
    Download the .dat file from the given URL, extract storm information,
    and update the strom directory file with the new or updated storm data.
    """
    # Download the data
    ...
    basin = file_name[1:3]  
    storm_number = file_name[3:5]  

    # Read existing  or create a new DataFrame
    ...
    if storm_year is None:
        ...
        # save the .dat file as parquet so that the app runs faster
    ...
    if storm_exists:
        ...
        print(f"Updated existing storm data for {storm_name} ({basin}{storm_number}, {storm_year}) in {directory_path}.")
        ...
        new_row = {
            ...
            'adeck_path': './forecast_data_parquet/' + file_name
        }
        df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)

        # save the new row in the directory
        df.to_parquet(directory_path, index=False)

        # delete .dat files from the storm forecast data parquet folder
        os.remove(f'forecast_data_parquet/{file_name}')

        print(f"Added new storm data for {storm_name} ({basin}{storm_number}, {storm_year}) to {directory_path}.")
        print(f'saved storm data to: ./forecast_data_parquet/' + file_name)
    
    # Save the updated directory
    df.to_parquet(directory_path, index=False)
    print(f"Storm data has been updated in {directory_path} for all sorms succesfully")
...
def check_and_download_file(file_url, save_path):
    ...
    if remote_last_modified and (local_last_modified is None or remote_last_modified > local_last_modified):
        ...
        dat_save_path=save_path.replace(".parquet",'.dat') #downloads and saves as a .dat file, but checks if the parquet is up-to-date
        download_file(file_url, dat_save_path)
        #Update storm_adeck_directory.csv
        download_and_update_storm_data(file_url, dat_save_path, directory_path)

    else:
        print(f"File {save_path} is up-to-date. Skipping download.")

def get_dat_files__main__(url):
    ...
    for link in soup.find_all('a'):
        ...
        if href.endswith('.dat'):
            ...
            file_name = os.path.basename(href)  # Extract the local file name from the URL
            file_name=file_name.replace('.dat','.parquet') #check if the parquet for the storm is up to date
...
for year in range(start_year, current_year+1):
    ...
    get_dat_files__main__(year_url)

"""
Need to now convert all the files that are in the .dat folder to partquet, for app speed
"""
#Get all the file names in the dat folder
dat_files=os.listdir(os.path.join(script_dir, "forecast_data"))
dat_names=dat_files.replace(".dat","")
parquet_files=os.listdir(os.path.jon(script_dir,"forecast_data_parquet"))
parquet_files=parquet_files.replace(".parquet","")

#Get all the file names in the dat folder
dat_files=os.listdir(os.path.join(script_dir, "forecast_data"))
dat_names= [f.replace('.dat', '') for f in dat_files]
parquet_files=os.listdir(os.path.join(script_dir,"forecast_data_parquet"))
parquet_names= [f.replace('.parquet', '') for f in dat_files]
