...
if not storm_adeck_data.empty:
    ...
    #remove empty basins
    forecast_basins_codes = [code for code in forecast_basins_codes if pd.notna(code)]
    # Create basin options for the dropdown
    forecast_basins = [
        {'label': basin_mapping.get(code, code), 'value': code}
        for code in forecast_basins_codes
        if pd.notna(basin_mapping.get(code, code)) and pd.notna(code)
    ]
...
def render_tab_content(tab):
    if tab == 'forecasts-tab':
        ...
        return html.Div([
            ...
            dcc.Graph(id='forecast-map',
                    style={'height': '800px'},
                        config={
                            'scrollZoom': True,  # Enables zooming with the mouse wheel
                            'displayModeBar': True,  # Show the mode bar with interaction options
                            'doubleClick': 'reset'  # Reset the zoom level on double-click
            })
        ])
...
def update_forecast_dates(selected_storm_id, selected_year):
    ...
    adeck_path = storm_row.iloc[0]['adeck_path_parquet']
    ...
    # Read the .parquet file and extract forecast dates
    storm_dat=gpd.read_parquet(adeck_full_path)
    dates=pd.to_datetime(storm_dat["DateTime"], format='%Y%m%d%H', errors='coerce')

    # Get unique dates
    unique_dates = sorted(dates.drop_duplicates())  # Remove duplicates
    if not unique_dates:
        return [], None
...
def update_forecast_map(selected_forecast_datetime, selected_storm_id, selected_year):
    ...
    adeck_gdf = gpd.read_parquet(adeck_full_path)
    #adeck_gdf = f.format_adeck_parquet(adeck_gdf)
...
# Critical configuration for external access
if __name__ == '__main__':
    # Bind to all network interfaces
    # Use a specific port that's not commonly used
    app.run_server(
        host='0.0.0.0',  # Allow connections from any interface
        port=8080,        # Choose a port not blocked by firewall
